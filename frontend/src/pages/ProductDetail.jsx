...
const ProductDetail = () => {
    ...
    useEffect(() => {
        const fetchProductData = async () => {
            try {
                const [productRes, reviewsRes, eligibilityRes] = await Promise.all([
                    api.get(`/products/${id}`),
            ...
            <Card className="card" sx={{ overflow: 'hidden', mb: 6 }}>
                <Grid container>
                    <Grid size={{ xs: 12, md: 6 }}>
                        <CardMedia
                            component="img"
                            image={getImageUrl(product.image_url) || 'https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?auto=format&fit=crop&q=80&w=800'}
                            alt={product.name}
                            sx={{ height: '100%', minHeight: 450, objectFit: 'cover' }}
                        />
                    </Grid>
                    <Grid size={{ xs: 12, md: 6 }}>
                        <CardContent sx={{ p: { xs: 3, md: 5 } }}>
                            ...
                            <Box sx={{ display: 'flex', gap: 2, mb: 4 }}>
                                ...
                                <Button
                                    ...
                                    component={Link}
                                    to="/messages"
                                    state={{ recipient_id: product.seller_id }}
